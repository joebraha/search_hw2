...
void DAAT(PostingsList *postings_lists, size_t num_terms);
...
// returns number of query terms found in lexicon (for deallocation)
size_t retrieve_postings_lists(char **terms, size_t num_terms,
                               PostingsList *postings_lists, FILE *index) {
...
// TODO: IMPLEMENT DAAT TRAVERSAL AND ASSOCIATED FUNCTIONS
void DAAT(PostingsList *postings_lists, size_t num_terms) {}
...
void load_lexicon(const char *filename) {
    ...
    while (fscanf(file, "%s %d %d %zu %zu %d %zu %zu %d", term, &count,
                  &start_d_block, &start_d_offset, &start_f_offset,
                  &last_d_block, &last_d_offset, &last_f_offset,
                  &last_did) == 9) {
        ...
        entry->num_blocks = 0;
        int prev_value =
            -1; // Initialize to a value that is less than any valid docID
        while (fscanf(file, "%d", &entry->last[entry->num_blocks]) == 1) {
            if (entry->last[entry->num_blocks] <= prev_value) {
                break; // Stop if the current value is not greater than the
                       // previous value- last array is only increasing
            }
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (strcasecmp(search_mode_input, "conjunctive") == 0 ||
            strcasecmp(search_mode_input, "c") == 0) { // for convenience
            search_mode = CONJUNCTIVE;
        } else if (strcasecmp(search_mode_input, "disjunctive") == 0 ||
                   strcasecmp(search_mode_input, "d") == 0) { // for convenience
            search_mode = DISJUNCTIVE;
        } else {
            printf("Invalid search mode. Please enter 'conjunctive' or "
                   "'disjunctive'.\n");
            continue;
        }
        ...
        // need to implement open_list, close_list, nextGEQ, and get_score
        // functions to do DAAT initialize top-10 heap

        DAAT(postings_lists, num_terms_processed);

        // Free allocated memory for terms and postings lists
        for (size_t i = 0; i < num_terms_processed; i++) {
            free(terms[i]);
